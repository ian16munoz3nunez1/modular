...
l1 = link(pi/2, 0.0, 0.5, 0.0)
l2 = link(0.0, 0.5, 0.0, 0.0)
l3 = link(0.0, 0.5, 0.0, 0.0)
l4 = link(0.0, 0.5, 0.0, 0.0)

youBot = YouBot(np.vstack((l1, l2, l3, l4)))

q = np.array([0.0, pi/4, -pi/4, 0.0]).reshape(-1, 1)
q_dot = np.array([0.0, 0.0, 0.0, 0.0]).reshape(-1, 1)
x_d = np.array([-1.0, 0.0, 1.0]).reshape(-1, 1)
...
while i <= N:
    ...
    e = x_d - x_i
    if abs(e[0]) <= 0.004 and abs(e[1]) <= 0.004 and abs(e[2]) <= 0.004:
        break
